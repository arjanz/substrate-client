...
ARG PROFILE=release
ARG REPOSITORY=totem-tech_totem-substrate
WORKDIR /rustbuilder
COPY . /rustbuilder
...
ARG PROFILE=release
ARG REPOSITORY=totem-tech_totem-substrate
COPY --from=builder /rustbuilder/$REPOSITORY/target/$PROFILE/totem-meccano /usr/local/bin
...
RUN mv /usr/share/ca* /tmp && \
	...
	mkdir -p /root/.local/share/totem-meccano && \
	ln -s /root/.local/share/totem-meccano /data
