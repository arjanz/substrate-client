...
ARG PROFILE=release
ARG REPOSITORY=laddernetwork_ladder
WORKDIR /rustbuilder
COPY . /rustbuilder
...
ARG PROFILE=release
ARG REPOSITORY=laddernetwork_ladder
COPY --from=builder /rustbuilder/$REPOSITORY/target/$PROFILE/ladder /usr/local/bin
...
RUN mv /usr/share/ca* /tmp && \
	...
	mkdir -p /root/.local/share/ladder && \
	ln -s /root/.local/share/ladder /data
RUN	rm -rf /usr/bin /usr/sbin

# FINAL PREPARATIONS
EXPOSE 30333 9933 9944
VOLUME ["/data"]
#CMD ["/usr/local/bin/ladder"]
