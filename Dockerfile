...
ARG PROFILE=release
ARG REPOSITORY=joystream-substrate-node-joystream
WORKDIR /rustbuilder
COPY . /rustbuilder
...
RUN cd /rustbuilder/$REPOSITORY/substrate-runtime-joystream && ./build.sh
...
ARG PROFILE=release
ARG REPOSITORY=joystream-substrate-node-joystream
COPY --from=builder /rustbuilder/$REPOSITORY/target/$PROFILE/joystream-node /usr/local/bin
